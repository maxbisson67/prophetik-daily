...
import { NovaCoachMatchHero, NovaCoachSingleAdvice } from "@src/nova/NovaCoachQuestionSections";
import {
  coachTitle,
  formatCoachBody,
  getNovaCoachMatchAdvice,
...
export default function NovaCoachMatchModal({
  visible,
  onClose,
  challengeId,
  gameId = null,
  ...
  }, [showIndicatorUi, domain, sport, indicatorsMeta, response, responseLang, homeAbbr, awayAbbr]);

  const showResponseBlock = !!response || (showIndicatorUi && !!indicatorsMeta);

  const matchAdvice = useMemo(
    () => getNovaCoachMatchAdvice({ sport, slot }),
    [sport, slot]
  );
  ...
  const askNova = useCallback(
    async (text, capability) => {
      ...
      if (!clean || busy || disabled || askDisabled || !gid || !canStartAsk()) return;
      ...
      const requestContext = {
        ...
        challengeId,
        gameId: gid,
        focusSlot: slot != null ? Number(slot) : undefined,
      };
      ...
      if (shouldFetchIndicators()) {
        novaCoachService({
          ...
          .catch((e) => {
            if (__DEV__) console.warn("[NovaCoachMatchModal] indicators error", e);
          });
      }
      ...
      try {
        ...
        if (__DEV__) console.warn("[NovaCoachMatchModal] error", e);
        setResponse(null);
        handleAskError(e, {
          indicatorsReceived,
          onQuotaExceeded: () => {
    ...
    [busy, disabled, askDisabled, gid, canStartAsk, lang, domain, sport, challengeId, slot, shouldFetchIndicators, handleAskError, showIndicatorUi]
  );
        ...
        >
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ color: colors.text, fontWeight: "900", fontSize: 17 }}>
                {i18n.t("novaCoach.matchModalTitle", { defaultValue: "Avis de Nova" })}
              </Text>
              <Text style={{ color: colors.subtext, fontWeight: "700", fontSize: 12 }}>
                {i18n.t("novaCoach.matchModalSubtitle", {
                  defaultValue: "Analyse pour ce match",
                })}
              </Text>
            </View>
            <TouchableOpacity
          ...
          >
            ...
            <NovaCoachSingleAdvice
              ...
              colors={colors}
              isDark={isDark}
              title={i18n.t("novaCoach.sectionMatchAdvice", {
                defaultValue: "Conseil sur ce match",
              })}
              subtitle={i18n.t("novaCoach.sectionMatchAdviceHint", {
                defaultValue: "Analyse personnalisée · IA",
              })}
              buttonLabel={i18n.t("novaCoach.singleAdviceButton", {
                defaultValue: "Obtenir l'avis de Nova",
              })}
            />
