        ...
        ListHeaderComponent={
          <View style={{ padding: 16, gap: 16 }}>
            ...
            <View style={{ padding: 14, borderWidth: 1, borderRadius: 12, backgroundColor:'#fff' }}>
              ...
              <Text style={{ fontSize: 34, fontWeight: '900', marginTop: 4 }}>{balance}</Text>
            </View>
            ...
            {/* Classements par groupe (optionnel ici) */}
            {groupsLB.map(lb => (
              <View key={lb.groupId} style={{ padding: 14, borderWidth: 1, borderRadius: 12, backgroundColor:'#fff' }}>
                <Text style={{ fontWeight: '800', fontSize: 16, marginBottom: 8 }}>
                  Classement — {lb.name}
                </Text>
                {lb.entries.length === 0 ? (
                  <Text style={{ color:'#6B7280' }}>Pas encore de crédits côté groupe.</Text>
                ) : lb.entries.map((e, i) => (
                  <View key={e.id} style={{ flexDirection:'row', justifyContent:'space-between', marginBottom: 6 }}>
                    <Text>
                      {i+1}. {e.displayName || e.id}
                    </Text>
                    <Text style={{ fontWeight:'800' }}>{e.balance ?? 0}</Text>
                  </View>
                ))}
              </View>
            ))}
